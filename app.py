...
def search_web(query):
    ...
    # Search the web with current informations
    results = ddgs.text(f"{query} {current_date}", max_results=10)
    # collect all the web results(tile, URL and Body) in a single string and return it
    if results:
        news_results = ""
...
web_search_agent = Agent(
    ...
    instructions='''Your role is to gather latest news articles on specified topics using 
